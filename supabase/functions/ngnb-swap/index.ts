  ...
  try {
    ...
    if (direction === 'bank-to-wallet') {
      ...
      const { data: updateResult, error: updateError } = await supabaseClient
        .rpc('atomic_deduct_ngnb_balance', {
          p_user_id: user.id,
          p_amount: amount
        });
      ...
      const { error: txError } = await supabaseClient
        ...
        .insert({
          user_id: user.id,
          transaction_type: 'ngnb_bank_to_wallet',
          amount: amount,
          ngnb_amount: amount,
          currency: 'NGNB',
          status: 'pending',
      ...
      const { data: updateResult, error: updateError } = await supabaseClient
        .rpc('atomic_credit_ngnb_balance', {
          p_user_id: user.id,
          p_amount: amount
        });
      ...
      const { error: txError } = await supabaseClient
        ...
        .insert({
          user_id: user.id,
          transaction_type: 'ngnb_wallet_to_bank',
          amount: amount,
          ngnb_amount: amount,
          currency: 'NGNB',
          status: 'completed',
    ...
    console.error('NGNB swap error:', error);
